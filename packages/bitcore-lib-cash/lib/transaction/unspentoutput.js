...
function UnspentOutput(data) {
  ...
  const outputIndex = data.vout ?? data.outputIndex;
  if (typeof outputIndex !== 'number') {
    throw new Error('Invalid outputIndex, received ' + outputIndex);
  }
  $.checkArgument(data.scriptPubKey != null || data.script != null || address != null,
    'Must provide the scriptPubKey, script, or address for that output!');
  const script = new Script(data.scriptPubKey || data.script || Script.fromAddress(address));
