...
HDPrivateKey.prototype._deriveWithNumber = function(index, hardened) {
  ...
  const cached = HDKeyCache.get(this.xprivkey, index, hardened);
  if (cached) {
    return cached;
  }
...
HDPrivateKey.prototype._buildFromObject = function(arg) {
  ...
  const buffers = {
    ...
    chainCode: (typeof arg.chainCode === 'string') ? BufferUtil.hexToBuffer(arg.chainCode) : arg.chainCode,
    privateKey: ((typeof arg.privateKey === 'string') && JSUtil.isHexa(arg.privateKey)) ? BufferUtil.hexToBuffer(arg.privateKey) : arg.privateKey,
